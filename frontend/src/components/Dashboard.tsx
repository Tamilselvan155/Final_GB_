...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const allBills = validBills.filter((bill: any) => !bill.bill_number?.startsWith('EXCH-'));
      const allExchangeBills = validBills.filter((bill: any) => bill.bill_number?.startsWith('EXCH-'));

      const billsValue = allBills.reduce(
        (sum, bill) => sum + (typeof bill.total_amount === 'number' ? bill.total_amount : parseFloat(bill.total_amount) || 0),
        0
      );
      const invoicesValue = validInvoices.reduce(
        (sum, invoice) => sum + (typeof invoice.total_amount === 'number' ? invoice.total_amount : parseFloat(invoice.total_amount) || 0),
        0
      );
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-6 border border-amber-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-600">
              {filterType === 'week' ? 'Last 8 Weeks' : filterType === 'month' ? 'Last 6 Months' : 'Custom Date Range'}
            </span>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-gray-500">All Time Summary</div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-gray-900">Financial Activity</h2>
                    ...
                    <div className="flex items-center space-x-3 flex-1">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          >
                            {isExchange ? 'Exchange Bill' : isInvoice ? 'Invoice' : 'Bill'}
                          </span>

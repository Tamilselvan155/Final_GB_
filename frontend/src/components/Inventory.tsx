...
import React, { useState, useEffect, useRef } from 'react';
import { Package, Plus, Search, Filter, CreditCard, Trash2, AlertTriangle, Scan, Download, Upload, Info, Scale, Barcode, User, Hash } from 'lucide-react';
import Database from '../utils/database';
import { Product } from '../types';
import { useLanguage } from '../contexts/LanguageContext';
import { useToast } from '../contexts/ToastContext';
            ...
            <div>
              ...
              <div className="flex space-x-2">
                ...
                <button
                  ...
                  disabled={isScanning}
                  className={`px-3 py-2 rounded-lg transition-colors ${
                    isScanning ? 'bg-amber-100 text-amber-600 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                  title="Click to focus barcode scanner"
                >
                  <Scan className="h-4 w-4" />
                ...
                <button
                  ...
                  disabled={isScanning || !formData.name.trim()}
                  className={`px-3 py-2 rounded-lg transition-colors ${
                    isScanning || !formData.name.trim() ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'
                  }`}
                  title="Generate barcode based on product name"
                >
                  <Package className="h-4 w-4" />
                ...
                <tr key={product.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      ...
                      {product.product_category ? (
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                          product.product_category === 'Men' ? 'bg-blue-100 text-blue-800' :
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                {viewingProduct.product_category ? (
                  <span className={`px-3 py-1 text-sm font-medium rounded-full ${
                    viewingProduct.product_category === 'Men' ? 'bg-blue-100 text-blue-800' :
                    viewingProduct.product_category === 'Women' ? 'bg-pink-100 text-pink-800' :
                    'bg-green-100 text-green-800'
                  }`}>
                    {viewingProduct.product_category}
                  </span>
